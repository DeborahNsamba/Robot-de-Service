...
class SecondScreen(Screen):
    ...
    def voice_command(self):
        """Starts listening for a voice command in a separate thread."""
    ...
    def _listen_for_command(self):
        ...
        try:
            with sr.Microphone() as source:
                # Update UI to indicate listening status
                Clock.schedule_once(lambda dt: setattr(self.ids.status, 'text', "Adjusting for noise..."))
                ...
                print("Listening...")
                audio = recognizer.listen(source, timeout=5, phrase_time_limit=4)  # Shortened phrase time
                
                # Recognize speech
                command = recognizer.recognize_google(audio).lower()
                print(f"Recognized command: {command}")

                # Process command on UI thread
                Clock.schedule_once(lambda dt: self.process_voice_command(command))

        except sr.WaitTimeoutError:
            Clock.schedule_once(lambda dt: setattr(self.ids.status, 'text', "No voice detected (Timeout)"))
        except sr.UnknownValueError:
            Clock.schedule_once(lambda dt: setattr(self.ids.status, 'text', "Could not understand the command"))
        except sr.RequestError as e:
            Clock.schedule_once(lambda dt: setattr(self.ids.status, 'text', f"Recognition service error: {e}"))
        except Exception as ex:
            error_message = f"Error: {ex}"
            Clock.schedule_once(lambda dt: setattr(self.ids.status, 'text', error_message))
